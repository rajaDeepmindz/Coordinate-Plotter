...
function drawLightCanvas({
  ...
  if (showBboxes) {
    fp.forEach((p) => {
      if (!p.bbox) return;
      ctx.strokeStyle =
        p.status === "Y" ? "rgba(220,38,38,0.5)" : "rgba(22,163,74,0.5)";
      ctx.lineWidth = 1.5;
      ctx.setLineDash([4, 3]);
      ctx.strokeRect(
        p.bbox.x1,
  ...
  fp.forEach((p) => {
    ...
    const isHov = gi === hoveredIdx,
      isSel = selectedRows.has(gi),
      isActive = gi === activeIdx;
    const r = isHov || isActive ? 7 : isSel ? 5.5 : 4;
    const accepted = p.status === "N";
    const color = accepted ? "#16a34a" : "#dc2626";
    const glowColor = accepted
      ? "rgba(22,163,74,0.15)"
      : "rgba(220,38,38,0.15)";

    if (isHov || isActive) {
      ctx.beginPath();
      ctx.arc(p.center.x, p.center.y, r + 7, 0, 2 * Math.PI);
      ctx.fillStyle = glowColor;
      ctx.fill();
    }
    ctx.fillStyle = "#fff";
    ctx.beginPath();
    ctx.arc(p.center.x, p.center.y, r + 1.5, 0, 2 * Math.PI);
    ctx.fill();
    ctx.fillStyle = color;
    ctx.beginPath();
    ...
    if (isActive) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(p.center.x, p.center.y, r + 4, 0, 2 * Math.PI);
      ctx.stroke();
    } else if (isHov || isSel) {
      ctx.strokeStyle = isSel ? "#f59e0b" : "#64748b";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.arc(p.center.x, p.center.y, r + 2, 0, 2 * Math.PI);
      ctx.stroke();
    }
    ...
    >
      <div
        ...
        style={{
          ...
          border: value ? "1.5px solid #2563eb" : "1.5px solid #b0bec5",
          transition: "all 0.2s",
        }}
      >
        <div
...
export default function App(): JSX.Element {
  ...
  const [activeTab, setActiveTab] = useState<"segments" | "data" | "grid">(
    "segments",
  );
  ...
  const navPoint = (dir: "up" | "down" | "left" | "right"): void => {
    ...
    const curFi = filteredPoints.findIndex(
      (p) => dataPoints.indexOf(p) === activeIdx,
    );
    const next =
      dir === "right" || dir === "down"
    ...
    const p = filteredPoints[next],
      canvas = canvasRef.current;
    if (canvas)
      setPan({
        x: canvas.width / 2 - p.center.x,
        y: canvas.height / 2 - p.center.y,
      });
  };
  ...
  const parseAndPlot = (): void => {
    ...
    result.points.forEach((p) => {
      p.relativeTime = p.timestampSec - base;
    });
  ...
  const plotSegments = (): void => {
    if (!segInput.trim()) {
      alert("Paste segment data first.");
      return;
    }
    try {
      setLineSegments(parseSegments(segInput));
    } catch (e) {
      alert("Bad segments: " + (e as Error).message);
    }
  };
  ...
  const stats = useMemo<Stats | null>(
    () => computeStats(dataPoints, filteredPoints),
  ...
  const activePoint: DataPoint | null =
    activeIdx >= 0 ? dataPoints[activeIdx] : null;
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="px-32 py-5 bg-gray-200"
          style={{
            flex: 1,
            position: "relative",
            overflow: "hidden",
        ...
        >
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    background: c,
                    boxShadow: `0 0 0 2px white, 0 0 0 3.5px ${c}55`,
                    flexShrink: 0,
                  }}
                />
                <span
            ...
            >
              ...
              {(
                [
                  ...
                  [
                    "Position",
                    `(${activePoint.center.x},${activePoint.center.y})`,
                    "#475569",
                  ],
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 11, fontWeight: 600, color: c, ...mono }}
                  >
                    {v}
                  </span>
                </div>
              ...
              <div
                style={{
                  ...
                  fontWeight: 700,
                  textAlign: "center",
                  color: activePoint.status === "N" ? "#15803d" : "#b91c1c",
                  ...mono,
                }}
              >
          ...
          <div style={{ position: "absolute", bottom: 14, right: 14 }}>
            ...
            <LightDPad
              onUp={() => navPoint("up")}
              onDown={() => navPoint("down")}
              onLeft={() => navPoint("left")}
              onRight={() => navPoint("right")}
            />
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setConfMin(+val.toFixed(2));
              }}
              className="w-16 text-[12px] px-1 py-[2px] bg-black border font-bold border-[#1e293b] text-sky-500 rounded text-center"
            />

            {/* 🔽 SLIDER */}
            <div className="flex-1 w-[300px] px-2">
              <LightDualRangeSlider
                min={0}
                max={1}
                step={0.01}
                valueMin={confMin}
          ...
          {/*<div className="flex items-center gap-1.5">
            <input
              ...
              onChange={(e) => {
                ...
                setMinMaskArea(Math.round(val));
              }}
              className="w-14 text-[10px] px-1 py-[2px] bg-[#020617] border border-[#1e293b] text-emerald-400 rounded text-center focus:outline-none focus:ring-1 focus:ring-emerald-500"
            />

            <div className="flex-1 w-[300px] px-2">
              <LightDualRangeSlider
                min={0}
                max={100000} // 🔁 adjust to your real max range
                step={1}
                valueMin={minMaskArea}
                valueMax={maxMaskArea}
                onChange={(lo, hi) => {
                  setMinMaskArea(Math.round(lo));
                  setMaxMaskArea(Math.round(hi));
                }}
              />
            </div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setMaxMaskArea(Math.round(val));
              }}
              className="w-14 text-[10px] px-1 py-[2px] bg-[#020617] border border-[#1e293b] text-lime-400 rounded text-center focus:outline-none focus:ring-1 focus:ring-lime-500"
            />
          </div> */}
          <div style={{ display: "flex", gap: 4, flexShrink: 0 }}></div>
      ...
      >
        ...
        >
          {[
            ...
            [
              "grid",
              `📊 Grid${dataPoints.length ? ` · ${dataPoints.length}` : ""}`,
            ],
          ].map(([key, label]) => (
            <button
              key={key}
              className={`cp-tab ${activeTab === key ? "on" : ""}`}
              onClick={() => setActiveTab(key as "segments" | "data" | "grid")}
            >
              {label}
            </button>
          ))}
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button className="btn btn-s" onClick={plotSegments}>
                  ▶ Plot
                </button>
                <button
                  className="btn btn-d"
        ...
        {activeTab === "data" && (
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button className="btn btn-s" onClick={parseAndPlot}>
                  ▶ Plot Data
                </button>
                <button className="btn btn-d" onClick={clearData}>
                  🗑 Clear
                </button>
              </div>
            </div>
                    ...
                    >
                      {(
                        ...
                        >
                          ...
                          <span style={{ color: c, fontWeight: 700, ...mono }}>
                            {v}
                          </span>
                        </div>
                      ))}
                    ...
                    {dataPoints.map((p, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <span
                              className={p.status === "Y" ? "tag-y" : "tag-n"}
                            >
                              {p.status === "Y" ? "YES" : "NO"}
                            </span>
                          </td>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {(
              [
                ...
                [
                  "Position",
                  `(${tooltip.point.center.x}, ${tooltip.point.center.y})`,
                  "#475569",
                ],
                ...
                [
                  "Δt",
                  `+${(tooltip.point.relativeTime || 0).toFixed(3)}s`,
                  "#94a3b8",
                ],
              ] as [string, string, string][]
            ).map(([k, v, c]) => (
          ...
          >
            <span
              style={{
                fontSize: 11,
                fontWeight: 700,
                color: tooltip.point.status === "N" ? "#15803d" : "#b91c1c",
                ...mono,
              }}
            >
