...
export default function App(): JSX.Element {
  ...
  /* ── Area auto-adjusts when confidence changes (linked filter) ── */
  useEffect(() => {
    ...
    if (areas.length > 0) {
      const lo = Math.min(...areas),
        hi = Math.max(...areas);
      setAreaMin((prev) => Math.max(lo, Math.min(hi, prev)));
      setAreaMax((prev) =>
        Math.max(lo, Math.min(hi, prev === areaAbsMax ? hi : prev)),
      );
      // If max was at ceiling, keep it at new ceiling
      setAreaMax(hi);
      setAreaMin(lo);
    }
  ...
  /* Draw segments: empty array if hidden */
